	...
	else
	{
		...
		while(direntry EXISTS)
		{
			...
			else if(DIRENT_IS_FILE)
			{
				newpath = getnewpath(dirname, direntry->d_name);
				tk = TKCreate(newpath);
				InsertTokensIntoIndex(id, tk, newpath);
				TKDestroy(tk);
				free(newpath);
			}
...
void InsertTokensIntoIndex(Index id, TokenizerT * tk, char * filepath)
{
	WordEnt went = NULL;
	...
	while(word EXISTS)
	{
		...
		rent = CreateRecord(relativepath);
...
char * getrelativepath(char * pathname)
{
	char * relativepath = pathname;
	//Gets a relative path name
	//If the pathname does not exist print error message and return NULL
	if(relativepath DOES_NOT_EXIST)
	{
		printf("Missing file path.");
		return NULL;
	}
	//If the path name starts with a slash, remove it as it is unecessary for our relative path
	if(relativepath[0] == '/')
		relativepath++;
	//Increment the pathname pointer until a null terminator or a '/' character is found
	while(*relativepath != 0 && *relativepath != '/')
		relativepath++;
	//If a slash was found, increment again to move the pointer passed the '/' and return this value
	//Else a slash was not found, return the original path name
	//	this occurs when the Index is originally passed a file instead of a directory, as this file will have no '/' characters
	if(*relativepath == '/') 
		return ++relativepath;
	else 
		return pathname;
